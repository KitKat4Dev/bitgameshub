...
// Movement constants
const SPEED = 0.08;
const SPRINT_SPEED = 0.93; // Adjusted so that when sprinting the player runs at ~200 km/h (~55.6 m/s)
const GRAVITY = 0.01;
...
export class PlayerControls {
  ...
  setupEventListeners() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key.toLowerCase() === "f" && window.gun && window.gun.playerHasGun) {
        window.gun.shoot();
      }
    });
    ...
    this.domElement.addEventListener("click", (e) => {
      if (window.gun && window.gun.playerHasGun) {
        window.gun.shoot();
      }
    });
  }
  ...
  processMovement() {
    ...
    if (!this.isMobile) {
      ...
      if (movement.length() > 0) {
        movement.normalize().multiplyScalar(isSprinting ? SPRINT_SPEED : SPEED);
      }
    } else {
